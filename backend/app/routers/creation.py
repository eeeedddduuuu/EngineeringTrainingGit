        ...
        if ark_key.startswith("ark-"):
            try:
                ...
                if "error" not in data:
                    ...
                    err = data["error"]
                    print(f"[Seedream] 不可用 ({err.get('code')}): {err.get('message')}")
            except Exception as exc:
                print(f"[Seedream] 调用失败: {exc}")

        # Seedream 不可用时回退到 PIL 文字封面
        if not seedream_ok:
            print("[Seedream] 回退到 PIL 文字封面")
            from PIL import Image, ImageDraw, ImageFont
            img = Image.new("RGB", (1080, 1920), (25, 20, 40))

            # 尝试用提示词生成简单封面
            lines = [l.strip() for l in image_prompt.strip().split("，") if l.strip()][:4]
            ...
            # 渐变背景条
            for i in range(1920):
                r_val = 25 + int(i / 1920 * 30)
                g_val = 20 + int(i / 1920 * 25)
                for x in range(1080):
                    if x % 8 == 0:
                        img.putpixel((x, i), (r_val, g_val, 40 + int(x / 1080 * 30)))
            ...
            draw.text((120, 1200), "🎬 AI 数字媒体创作助手", fill=(200, 200, 220), font=font_sm)
            draw.text((120, 1300), "由灵犀创作助手自动生成", fill=(150, 150, 170), font=font_sm)
            img.save(img_path)
            print(f"[PIL] 文字封面已生成: {img_path}")
