...
# ── 配置 ──────────────────────────────────────────────────
def _get_api_key():
    # 优先级：ARK_VISION_KEY（多模态专用）→ ARK_API_KEY（生图Key，兜底）→ app.config → 空
    key = os.environ.get("ARK_VISION_KEY", "")
    ...
    if key and key.strip():
        return key.strip()
    try:
        from app.config import ARK_VISION_KEY as _cfg_key
        if _cfg_key and _cfg_key.strip():
            os.environ["ARK_VISION_KEY"] = _cfg_key
